...
import { icon as renderIcon } from './icons.mjs';
...
/** Load the HEIC→JPEG converter on demand (only when a HEIC is actually picked), so the
 *  base app stays light. Resolves to window.heic2any, or null if it can't be fetched. */
let heicLoader = null;
function ensureHeic() {
...
async function convertHeic(file) {
  const heic2any = await ensureHeic();
  if (!heic2any) return null;
  const out = await heic2any({ blob: file, toType: 'image/jpeg', quality: 0.92 });
  return Array.isArray(out) ? out[0] : out; // a multi-image HEIC yields an array
}
...
function imgFrom(url, revoke) {
  return new Promise((resolve) => {
    const img = new Image();
    img.onload = async () => {
      try { if (img.decode) await img.decode(); } catch { /* onload already fired; use it */ }
...
/** Try every decode path a real phone might need, newest/cheapest first. */
async function decodeAny(file) {
  try { const b = await createImageBitmap(file, { imageOrientation: 'from-image' }); if (b && b.width) return b; } catch { /* try next */ }
  try { const b = await createImageBitmap(file); if (b && b.width) return b; } catch { /* try next */ }
  try { const img = await imgFrom(URL.createObjectURL(file), true); if (img) return img; } catch { /* try next */ }
  try { const img = await imgFrom(await fileToDataUrl(file), false); if (img) return img; } catch { /* give up */ }
  return null;
}

/** Decode a picked file, honouring EXIF rotation, and shrink it for memory. Handles
 *  HEIC/HEIF from the gallery by converting to JPEG first when needed. */
async function loadPhoto(file) {
  let source;
  if (looksHeic(file)) {
    // Convert first; if that can't run, still try a direct decode in case it's mislabelled.
    source = (await tryHeicDecode(file)) || (await decodeAny(file));
  } else {
    // Decode directly; if every path fails it may be a HEIC with an image/* or empty type.
    source = (await decodeAny(file)) || (await tryHeicDecode(file));
  }
...
async function tryHeicDecode(file) {
  try {
    const jpeg = await convertHeic(file);
    if (jpeg) return await decodeAny(jpeg);
  } catch { /* converter unavailable or failed */ }
  return null;
}
...
function dismissSwipeHint() {
  ...
  // Go to `spent` (invisible but KEEPS its fixed-height box) — never display:none here,
  // or the coverflow below would jump up into the freed space.
  if (reduceMotion()) { el.classList.add('spent'); return; }
  el.classList.add('popping'); // burst the arrow, fingertip and confetti bits
  setTimeout(() => { el.classList.add('spent'); el.classList.remove('popping'); }, 460);
}

// Tapping the check commits the design and reveals Save/Print, so the step guide
// shrinks into nothing — once and for all. It never comes back this session, not even
// for a fresh set of four photos.
let stepGuideRetired = false;
function popStepGuide() {
  const g = $('stepGuide');
  if (!g || g.classList.contains('hidden')) return;
  stepGuideRetired = true;
...
function showStepGuide() {
  if (stepGuideRetired) return; // the guide only ever appears once
  const g = $('stepGuide');
  if (!g) return;
...
function updatePickButton() {
  ...
  // it returns the next time a fresh set of four brings the coverflow back. Off step 2
  // it's fully hidden (display:none); once dismissed on step 2 it goes `spent` —
  // invisible but keeping its box so the pictures below never shift.
  if (!full) swipeHintDismissed = false;
  const hint = $('swipeHint');
  if (!hint.classList.contains('popping')) {
    hint.classList.toggle('hidden', !full);
    hint.classList.toggle('spent', full && swipeHintDismissed);
  }
  if (full) layoutGoo();
}
...
function showJob(job) {
  ...
  if (job.status === 'printing' || job.status === 'claimed') {
    return showResult({
      ...
      body: session.dryRun
        ...
        : `Your ${job.copies === 1 ? 'copy is' : `${job.copies} copies are`} printing now${onPrinter(job)}. Hang tight, we'll tell you the moment it's ready.`,
      image: job.image,
      busy: true,
    });
  }
  ...
  if (job.status === 'cancelled') {
    return showResult({
      ...
      body: 'The host cancelled this print. You can still save it to your phone.',
      image: job.image,
    });
  }
...
function updateQueuePill(job) {
  ...
  let iconName = 'printer', title = printTag(job) || 'Print', sub = '';
  if (stillWaiting(job)) {
    iconName = 'printer';
    if (job.agentOnline === false) {
      sub = 'queued';
      ...
      const place = job.queue.position <= 1 ? 'next' : `#${job.queue.position}`;
      sub = `${place} · ${compactEta(seconds)}`;
    }
  ...
  $('qwIcon').innerHTML = renderIcon(iconName);
...
function renderJob(job) {
  ...
  if (sendRaf != null && (job.status === 'printing' || job.status === 'claimed')) {
    ...
    setResultText('printer', session.dryRun ? 'Printing (dry run)' : 'Printing now!',
      session.dryRun
        ? 'Dry-run mode. Nothing goes to a real printer.'
        : `${copies(['Your copy is', `Your ${job.copies} copies are`])} printing now${onPrinter(job)}. Hang tight, we'll tell you the moment it's ready.`);
    $('resultDone').disabled = false;
    return;
  }
  if (sendRaf != null && (job.status === 'done' || job.status === 'queued')) {
    setAnimPhase('done');
    setResultText('success', session.dryRun ? 'Saved (dry run)' : 'All done!',
      session.dryRun
        ? 'The booth is in dry-run mode, so nothing was sent to a real printer.'
        : `${copies(['Your print is', 'Your prints are'])} ready. ${session.remote ? 'Collect it from the booth.' : 'Grab it from the tray.'}`);
    if (!session.dryRun) burstConfetti();
    $('resultDone').disabled = false;
    return;
  }
...
function trackQueue(job) {
  ...
  queuePoll = setInterval(async () => {
    ...
    try {
      ...
      next = payload.job;
      next.agentOnline = payload.agentOnline; // keep the offline-queue wording accurate
    } catch {
...
function showResult({ icon: iconName, title, body, image, busy, celebrate }) {
  stopSendAnim(); // any non-sending result screen ends the transmit animation
  $('resultEmoji').innerHTML = renderIcon(iconName);
...
// A confetti burst for the celebratory "all done" state (replaces the old party emoji).
let lastConfetti = 0;
function burstConfetti() {
  const card = document.querySelector('#result .result-card');
  if (!card || reduceMotion()) return;
  const now = Date.now();
  if (now - lastConfetti < 2500) return; // one burst per finish, not per poll tick
  lastConfetti = now;
  const colors = ['#6a5cff', '#a24bff', '#ff4d97', '#ff7a4d', '#ffd23f', '#4aa8c9'];
  const layer = document.createElement('div');
  layer.className = 'confetti-layer';
  card.appendChild(layer);
  const rect = card.getBoundingClientRect();
  const cx = rect.width / 2;
  const cy = Math.min(rect.height * 0.3, 130);
  for (let i = 0; i < 44; i += 1) {
    ...
    const dy = Math.sin(ang) * dist - 30; // bias upward at first
    const fall = rect.height * 0.7 + Math.random() * 90;
    bit.animate(
      [
        { transform: 'translate(-50%,-50%) rotate(0deg)', opacity: 1 },
        { transform: `translate(calc(-50% + ${dx}px),calc(-50% + ${dy}px)) rotate(${Math.random() * 360}deg)`, opacity: 1, offset: 0.35 },
        { transform: `translate(calc(-50% + ${dx * 1.1}px),calc(-50% + ${dy + fall}px)) rotate(${Math.random() * 720}deg)`, opacity: 0 },
      ],
      { duration: 1100 + Math.random() * 700, easing: 'cubic-bezier(.2,.6,.2,1)' },
    ).onfinish = () => bit.remove();
  }
...
function setResultText(iconName, title, body) {
  $('resultEmoji').innerHTML = renderIcon(iconName);
  $('resultTitle').textContent = title;
  $('resultBody').textContent = body;
  $('result').classList.remove('hidden');
...
function beginSending(blob) {
  $('resultEmoji').innerHTML = renderIcon('offline');
...
async function doPrint() {
  ...
  $('queuePill').classList.add('hidden');
  showResult({ icon: 'printer', title: 'Building your print…', body: 'Rendering at 600 DPI.', busy: true });
  ...
  try {
    ...
    showResult({ icon: 'error', title: 'Could not build the print', body: err.message });
    return;
  }
  ...
  try {
    ...
    while (status === 402 && data && data.codeError) {
      ...
      if (!printCode) {
        showResult({ icon: 'ticket', title: 'Print cancelled', body: 'No code entered. You can still save the photo to your phone.' });
        return;
      }
    ...
    data.job.agentOnline = data.agentOnline; // so the queue screen can be honest if offline
    trackQueue(data.job);
  } catch (err) {
    showResult({
      icon: 'wifi',
